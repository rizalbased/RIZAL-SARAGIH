...
$identifier = trim($input['emailOrUsername'] ?? $input['email'] ?? $input['username'] ?? '');
...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT id, username, email, password_hash, display_name, avatar, cover_image, bio,
    ...
    if ((int)$user['email_verified'] === 0) {
        ...
        echo json_encode([
            ...
            'email' => $user['email'],
            'message' => 'Silakan verifikasi alamat email Anda terlebih dahulu sebelum masuk ke MKVERSE.',
            'user' => [
                'id' => $user['id'],
