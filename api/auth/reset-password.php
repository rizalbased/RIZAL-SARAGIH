...
$password = $input['password'] ?? $input['pass'] ?? $input['newPassword'] ?? '';
$confirmPassword = $input['confirmPassword'] ?? $input['confirmPass'] ?? '';
...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT id, username, email, display_name, reset_expires_at
        FROM users 
